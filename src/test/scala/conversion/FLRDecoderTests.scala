...
class FLRDecoderTests extends AnyFlatSpec with Matchers:
  ...
  "BirthDay" should "construct from valid values" in:
    "19870101".as[BirthDay] should be (Valid(BirthDay(LocalDate.of(1987, 1, 1))))
  ...
  "Debit" should "construct from valid values" in:
    "10000".as[Debit]               should be (Valid(Debit(100.00d)))
    " 12345 ".as[Debit]             should be (Valid(Debit(123.45d)))
    s"${Double.MaxValue}".as[Debit] should be (Valid(Debit(Double.MaxValue / 100)))
